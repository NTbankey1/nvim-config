return {
  ...
  config = function()
    ...
    require('session_manager').setup({
      ...
      -- CurrentDir: tự load session khi mở nvim trong thư mục dự án
      ...
      autosave_only_in_session = false,  -- lưu ngay cả khi chưa load session trước
    ...
    local map = function(k, cmd, desc)
      vim.keymap.set('n', k, '<cmd>' .. cmd .. '<CR>', { desc = desc, silent = true })
    end
    map('<leader>Ss', 'SessionManager save_current_session',  'Save session')
    map('<leader>Sl', 'SessionManager load_session',          'Load session (pick)')
    map('<leader>Sd', 'SessionManager delete_session',        'Delete session')
    map('<leader>SC', 'SessionManager load_current_dir_session', 'Load session (current dir)')

    -- Tự động lưu session khi thoát Neovim
    vim.api.nvim_create_autocmd('VimLeavePre', {
      callback = function()
        -- Chỉ lưu nếu có file thực đang mở
        local bufs = vim.fn.getbufinfo({ buflisted = 1 })
        local has_file = false
        for _, b in ipairs(bufs) do
          if b.name and b.name ~= '' and not b.name:match('^term://') then
            has_file = true
            break
          end
        end
