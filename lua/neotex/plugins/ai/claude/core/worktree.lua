...
function M._get_claude_sessions()
  ...
  local cmd = "cc-nvim --resume --list 2>/dev/null || echo ''"
  local result = vim.fn.system(cmd)
  ...
  if result == "" or result:match("unknown option") then
    ...
    cmd = "cc-nvim --resume < /dev/null 2>&1 | head -50"
...
function M._resume_claude_session(session)
  ...
  if has_claude_code then
    ...
    if is_terminal then
      ...
      vim.api.nvim_feedkeys("cc-nvim --resume " .. session.id .. "\n", "n", false)
    else
      -- Otherwise, open a new terminal
      vim.cmd("terminal cc-nvim --resume " .. session.id)
      vim.cmd("startinsert")
    end
  end
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      map("i", "<C-t>", function()
        ...
        if selection then
          ...
          local cmd = terminal_cmds.spawn_tab(
            session.path,
            string.format("bash -c 'cc-nvim --resume %s'", session.id)
          )
