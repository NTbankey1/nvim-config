...
function M.get_session_id_by_number(number)
  -- Run cc-nvim --resume in non-interactive mode to get session list with IDs
  local output = vim.fn.system("cc-nvim --resume --non-interactive 2>&1 || true")
  ...
  local simulate_cmd = string.format("echo '%d' | cc-nvim --resume 2>&1 | head -1", number)
...
function M.show_picker()
  -- Get session list
  local output = vim.fn.system("cc-nvim --resume --list 2>/dev/null || cc-nvim --resume 2>&1 | head -20")
