...
function M.setup()
  ...
  -- ============================================================
  -- FILE RESOLVER: mở file từ Claude Code output
  -- Xử lý format: path/to/file.ext:line:col
  --   hoặc:       path/to/file.ext:line
  --   hoặc:       `path/to/file.ext` (backticks)
  -- Tự động tìm git root nếu path là relative
  -- ============================================================

  function _G.open_file_under_cursor()
    -- Lấy toàn bộ word dưới con trỏ (bao gồm dấu : và .)
    ...
    -- Xoá backticks, quotes, dấu phẩy, dấu chấm than cuối
    cursor_word = cursor_word:gsub("^[`'\"]+", ""):gsub("[`'\",;.:!?]+$", "")

    -- Parse file path, line, column từ format "path:line:col" hoặc "path:line"
    local filepath = cursor_word
    local line = nil
    local col = nil

    -- Thử pattern path:line:col
    local path_part, line_str, col_str = cursor_word:match("^(.+%.%w+):(%d+):(%d+)$")
    if path_part then
      filepath = path_part
      line = tonumber(line_str)
      col = tonumber(col_str)
    else
      -- Thử pattern path:line
      path_part, line_str = cursor_word:match("^(.+%.%w+):(%d+)$")
      if path_part then
        filepath = path_part
        line = tonumber(line_str)
      end
    ...
    -- Debug
    vim.notify(string.format("Looking for: %s (line=%s, col=%s)", filepath, tostring(line or "?"), tostring(col or "?")), vim.log.levels.DEBUG)

    -- Kiểm tra file tồn tại trực tiếp
    if vim.fn.filereadable(filepath) == 1 then
      vim.cmd(string.format("edit +%d %s", line or 1, vim.fn.fnameescape(filepath)))
      if col then vim.fn.cursor(line or 1, col) end
      return
    end

    -- Thử tìm trong git root của buffer hiện tại hoặc cwd
    local dirs = {}
    local bufpath = vim.api.nvim_buf_get_name(0)
    local bufdir
    if bufpath and bufpath ~= "" then
      bufdir = vim.fn.fnamemodify(bufpath, ":h")
      local git_root = vim.fn.system("cd " .. vim.fn.shellescape(bufdir) .. " && git rev-parse --show-toplevel 2>/dev/null")
      if git_root and git_root ~= "" then
        table.insert(dirs, vim.fn.trim(git_root))
      end
      table.insert(dirs, bufdir)
    end
    local cwd_root = vim.fn.system("git rev-parse --show-toplevel 2>/dev/null")
    if cwd_root and cwd_root ~= "" then
      table.insert(dirs, vim.fn.trim(cwd_root))
    end
    ...
    for _, dir in ipairs(dirs) do
      if dir and dir ~= "" and not seen[dir] then
        seen[dir] = true
        local fullpath = dir .. "/" .. filepath
        if vim.fn.filereadable(fullpath) == 1 then
          vim.cmd(string.format("edit +%d %s", line or 1, vim.fn.fnameescape(fullpath)))
          if col then vim.fn.cursor(line or 1, col) end
          return
        end
      end
    ...
    -- Thử tìm file trong cwd tree (chỉ tìm tên file)
    local filename = vim.fn.fnamemodify(filepath, ":t")
    if filename and filename ~= "" then
      local found = vim.fn.findfile(filename, "**")
      if found and found ~= "" then
        vim.cmd(string.format("edit +%d %s", line or 1, vim.fn.fnameescape(found)))
        if col then vim.fn.cursor(line or 1, col) end
        return
      end
    end

    vim.notify("Cannot find file: " .. filepath, vim.log.levels.WARN)
  end
  ...
  function _G.set_terminal_keymaps()
    ...
    -- ⭐ Mở file từ Claude Code NGAY TRONG TERMINAL MODE
    -- Khi Claude output "đã sửa which-key.lua:42", đặt cursor lên đó, gõ <C-g>
    -- Tự động: thoát terminal → mở file → quay lại terminal
    buf_map(0, "t", "<C-g>", "<Cmd>call feedkeys(\"\\<C-\\\\>\\<C-N>:lua _G.open_file_under_cursor()\\<CR>i\")<CR>", "Open file at cursor")
    buf_map(0, "n", "gf", "<Cmd>lua _G.open_file_under_cursor()<CR>", "Open file at cursor")
    buf_map(0, "n", "gF", "<Cmd>lua _G.open_file_under_cursor()<CR>", "Open file at cursor (with line)")
  end
  ...
  --------------------------------------------------
  -- VSCODE COMPATIBILITY KEYMAPS (LSP & EDITING) --
  --------------------------------------------------
  -- Advanced Code Navigation & Definitions
  -- Tự động kiểm tra LSP server trước khi gọi, báo lỗi thân thiện nếu không có
  map("n", "gd", function()
    local ok = vim.lsp.buf.definition()
    if ok == nil then vim.notify("No LSP server active for this file", vim.log.levels.WARN) end
  end, {}, "Reveal Definition (VSCode)")
  map("n", "gh", function()
    local clients = vim.lsp.get_clients({ bufnr = 0 })
    if #clients == 0 then vim.notify("No LSP server active for this file", vim.log.levels.WARN) return end
    vim.lsp.buf.hover()
  end, {}, "Show Hover (VSCode)")
  map("n", "gi", function()
    local clients = vim.lsp.get_clients({ bufnr = 0 })
    if #clients == 0 then vim.notify("No LSP server active for this file", vim.log.levels.WARN) return end
    vim.lsp.buf.implementation()
  end, {}, "Go to Implementation (VSCode)")
  map("n", "gq", function()
    local clients = vim.lsp.get_clients({ bufnr = 0 })
    if #clients == 0 then vim.notify("No LSP server active for this file", vim.log.levels.WARN) return end
    vim.lsp.buf.code_action()
  end, {}, "Quick Fix (VSCode)")
  map("n", "gr", "<cmd>Telescope lsp_references<CR>", {}, "Reference Search (VSCode)")
  map("n", "gt", function()
    local clients = vim.lsp.get_clients({ bufnr = 0 })
    if #clients == 0 then vim.notify("No LSP server active for this file", vim.log.levels.WARN) return end
    vim.lsp.buf.type_definition()
  end, {}, "Go to Type Definition (VSCode)")
  map("n", "go", "<cmd>Telescope lsp_document_symbols<CR>", {}, "Go to Symbol (VSCode)")
  map("n", "gO", "<cmd>Telescope lsp_workspace_symbols<CR>", {}, "Show All Symbols (VSCode)")

  -- Rename symbol (thường dùng gần với LSP nav)
  map("n", "<leader>rn", function()
    local clients = vim.lsp.get_clients({ bufnr = 0 })
    if #clients == 0 then vim.notify("No LSP server active for this file", vim.log.levels.WARN) return end
    vim.lsp.buf.rename()
  end, {}, "Rename Symbol (VSCode)")

  -- Diagnostics Navigation
  map("n", "<leader>j", "<cmd>lua vim.diagnostic.goto_next()<CR>", {}, "Next Diagnostic (VSCode)")
  map("n", "<leader>k", "<cmd>lua vim.diagnostic.goto_prev()<CR>", {}, "Prev Diagnostic (VSCode)")
  map("n", "<leader>l", "<cmd>Telescope diagnostics<CR>", {}, "Problems Focus (VSCode)")

  -- Code Editing & Search
  map("n", "<leader>/", "<cmd>Telescope live_grep<CR>", {}, "Find in Files (VSCode)")
  map("n", "<leader>fm", "<cmd>lua require('conform').format({ async = true, lsp_fallback = true })<CR>", {}, "Format Document (VSCode)")
  -- NOTE: <leader>rn was moved above (next to gd/gh/gi LSP keys for consistency)
